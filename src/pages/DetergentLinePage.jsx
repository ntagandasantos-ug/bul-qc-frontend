...
// Detergent Line Inspection — same design as SoapLinePage
// ============================================================
...
const GR = '#16A34A';
const RD = '#DC2626';
const AM = '#D97706'; // amber for high weight
const G  = '#FFB81C';

// ── Brand / pack-size catalogue ───────────────────────────
const CATALOGUE = {
  'White Star Magic': [
    { size:'45g',   type:'Pouch',  prefix:'00102', lines:16, targetMin:275,  targetMax:283  },
    { size:'500g',  type:'Pouch',  prefix:'00103', lines:5,  targetMin:495,  targetMax:505  },
    { size:'1kg',   type:'Pouch',  prefix:'00104', lines:5,  targetMin:990,  targetMax:1010 },
    { size:'250g',  type:'Bucket', prefix:'00105', lines:5,  targetMin:null, targetMax:null },
    { size:'500g',  type:'Bucket', prefix:'00106', lines:5,  targetMin:495,  targetMax:505  },
    { size:'1kg',   type:'Bucket', prefix:'00107', lines:5,  targetMin:990,  targetMax:1010 },
    { size:'3kg',   type:'Bag',    prefix:'00108', lines:5,  targetMin:2990, targetMax:3010 },
    { size:'5kg',   type:'Bag',    prefix:'00109', lines:5,  targetMin:4990, targetMax:5010 },
    { size:'10kg',  type:'Bag',    prefix:'00101', lines:5,  targetMin:9990, targetMax:10010},
  ],
  'Bleu Breeze': [
    { size:'3kg',  type:'Bag', prefix:'00205', lines:5, targetMin:2990, targetMax:3010 },
    { size:'5kg',  type:'Bag', prefix:'00206', lines:5, targetMin:4990, targetMax:5010 },
    { size:'10kg', type:'Bag', prefix:'00207', lines:5, targetMin:9990, targetMax:10010},
  ],
};

const BRANDS = Object.keys(CATALOGUE);

// Generate line options based on pack size
const getLines = (brand, size) => {
  const entry = (CATALOGUE[brand]||[]).find(e=>e.size===size);
  const count = entry?.lines || 5;
  return Array.from({length:count},(_,i)=>`Line ${i+1}`);
};

// Auto-generate batch number
const genBatch = (brand, size, dateStr, shift) => {
  const entry = (CATALOGUE[brand]||[]).find(e=>e.size===size);
  if (!entry?.prefix || !dateStr) return '';
  const day   = format(new Date(dateStr),'dd');
  const digit = shift==='day'?'1':'2';
  return entry.prefix + day + digit;
};
...
const calcBestBefore = (batchNumber, dateStr) => {
  ...
  try {
    ...
    const bb = addYears(prod, 2);
    return format(bb,'yyyy-MM-dd');
  } catch { return ''; }
};

// Target weight display string
const targetStr = (brand, size) => {
  const e = (CATALOGUE[brand]||[]).find(en=>en.size===size);
  if (!e?.targetMin) return '—';
  return `${e.targetMin} – ${e.targetMax} g`;
};

// Deviation + status
const calcDeviation = (brand, size, actual) => {
  const e = (CATALOGUE[brand]||[]).find(en=>en.size===size);
  if (!e?.targetMin || actual===''||actual===null||isNaN(actual)) return { dev:null, status:null };
  const nominal = (e.targetMin + e.targetMax) / 2;
  const dev     = parseFloat(actual) - nominal;
  let status    = 'pass';
  if (parseFloat(actual) < e.targetMin) status = 'low';
  if (parseFloat(actual) > e.targetMax) status = 'high';
  return { dev: parseFloat(dev.toFixed(1)), status };
};

// Smart remark suggestion from deviation status
...
function ActionCell({ value, onChange, options, onNewOption }) {
  ...
  const stdOpts = options['action'] || ['Pass','Adjusted','Line stopped for correction','Supervisor informed','Filler adjusted','Sealer recalibrated','No action required'];

  if (custom) return (
    <div>
      <SmartCell paramKey="action" value={value} onChange={v=>{ onChange(v); setCustom(false); }} options={options} onNewOption={onNewOption} placeholder="Type action..."/>
      <button type="button" onClick={()=>setCustom(false)} style={{ background:'none', border:'none', color:'#94A3B8', fontSize:'10px', cursor:'pointer', fontFamily:'inherit', padding:'1px 0' }}>↩ back to list</button>
    </div>
  );
...
export default function DetergentLinePage() {
  ...
  // ── Form state ────────────────────────────────────────────
  const emptyRecord = ()=>({
    line_number:'Line 1', brand_name:'', pack_size:'', pack_type:'',
  ...
  const setR = (i,k,v)=>setForm(p=>{
    ...
    // Auto-fill batch when brand+size set
    if(k==='brand_name'||k==='pack_size') {
      const brand  = k==='brand_name'?v:records[i].brand_name;
      const size   = k==='pack_size'?v:records[i].pack_size;
      const entry  = (CATALOGUE[brand]||[]).find(e=>e.size===size);
      updated.pack_type    = entry?.type||'';
      updated.batch_number = genBatch(brand,size,date,shift);
      updated.best_before  = calcBestBefore(updated.batch_number,date);
      // Reset line to Line 1 but update line options
      updated.line_number  = 'Line 1';
    }

    // Auto-fill batch when size changes
    if(k==='actual_weight') {
      const { dev, status } = calcDeviation(records[i].brand_name, records[i].pack_size, v);
      updated._dev    = dev;
      updated._status = status;
      // Suggest remark if empty
  ...
  const openEdit = (entry)=>{
    const records=(entry.detergent_inspection_records||[]).map(r=>({
      line_number  : r.line_number||'Line 1',
  ...
  const handleSave = async()=>{
    ...
    if(form.records.some(r=>!r.brand_name||!r.pack_size)) { toast.warning('Select brand and pack size for all rows'); return; }
    setSaving(true);
    try {
      let entryId = editEntryId;
      if(editEntryId) {
        await supabase.from('detergent_inspection_entries').update({ inspection_time:form.inspection_time, inspector_name:form.inspector_name }).eq('id',editEntryId);
      ...
      for(const r of form.records) {
        const { dev, status } = calcDeviation(r.brand_name, r.pack_size, r.actual_weight);
  ...
  const hasOOS = entries.some(e=>(e.detergent_inspection_records||[]).some(r=>r.weight_status==='high'||r.weight_status==='low'));
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8FAFC', paddingBottom:'60px' }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${P},${PM})`, padding:'14px 24px', color:'#fff', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'10px' }}>
        <div>
          <h1 style={{ margin:0, fontSize:'18px', fontWeight:'900' }}>🧴 Detergent Line Inspection</h1>
            ...
            {showForm && (
              <div style={{ background:'#fff', borderRadius:'14px', border:`2px solid ${editEntryId?'#FED7AA':PL}`, marginBottom:'18px', overflow:'hidden', boxShadow:'0 4px 16px rgba(107,33,168,0.07)' }}>
                  ...
                  <table style={{ borderCollapse:'collapse', width:'100%', fontSize:'12px' }}>
                    <thead>
                      <tr>
                        <TH w="88px">Line</TH>
                        <TH w="145px">Brand</TH>
                        <TH w="78px">Pack Size</TH>
                        <TH w="68px">Type</TH>
                        <TH w="105px">Batch No.</TH>
                        <TH w="92px">Appearance</TH>
                        <TH w="82px">Sealing</TH>
                        <TH w="118px" center>Target Weight</TH>
                    ...
                    <tbody>
                      {form.records.map((r,idx)=>{
                        const brandObj    = CATALOGUE[r.brand_name]||[];
                        const packEntry   = brandObj.find(e=>e.size===r.pack_size);
                        const lineOptions = getLines(r.brand_name, r.pack_size);
                        const { dev, status } = calcDeviation(r.brand_name, r.pack_size, r.actual_weight);
                        ...
                        return (
                          <tr key={idx} style={{ background:rowBg }}>
                            ...
                            <td style={{ padding:'5px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                              <select value={r.line_number} onChange={e=>setR(idx,'line_number',e.target.value)}
                            ...
                            <td style={{ padding:'5px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                              <select value={r.pack_size} onChange={e=>setR(idx,'pack_size',e.target.value)} disabled={!r.brand_name}
                                ...
                                <option value="">—</option>
                                {(CATALOGUE[r.brand_name]||[]).map(e=><option key={e.size} value={e.size}>{e.size}</option>)}
                              </select>
                            </td>
                            {/* Type (auto-filled) */}
                            <td style={{ padding:'5px 7px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', color:'#64748B', fontSize:'11px', fontWeight:'600', whiteSpace:'nowrap' }}>
                              {r.pack_type||'—'}
                            </td>
                            {/* Batch number (auto-filled, editable) */}
                            <td style={{ padding:'5px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                              <input type="text" value={r.batch_number}
                                onChange={e=>setR(idx,'batch_number',e.target.value)}
                                style={{ width:'100%', border:'1px solid #E2E8F0', borderRadius:'5px', padding:'5px 6px', fontSize:'11px', fontFamily:'monospace', background:'#F5F3FF', outline:'none', boxSizing:'border-box', color:P, fontWeight:'700' }}
                              />
                            </td>
                            {/* Appearance */}
                            ...
                            <td style={{ padding:'6px 8px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', textAlign:'center' }}>
                              <span style={{ fontSize:'11px', color:'#D97706', fontWeight:'700', background:'#FFFBEB', padding:'2px 7px', borderRadius:'5px', border:'1px solid #FED7AA', whiteSpace:'nowrap' }}>
                                {targetStr(r.brand_name,r.pack_size)}
                              </span>
                            </td>
                            {/* Actual weight */}
                            <td style={{ padding:'4px 6px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', textAlign:'center' }}>
                              <input type="number" value={r.actual_weight} onChange={e=>setR(idx,'actual_weight',e.target.value)}
                                placeholder="g"
                                style={{ width:'80px', border:`1px solid ${status===null?'#E2E8F0':status==='pass'?'#86EFAC':status==='high'?'#FED7AA':'#FECACA'}`, borderRadius:'5px', padding:'5px 6px', fontSize:'12px', fontFamily:'inherit', background:status===null?'#fff':status==='pass'?'#F0FDF4':status==='high'?'#FFFBEB':'#FFF5F5', outline:'none', textAlign:'center' }}
                              />
                            </td>
                            ...
                            {/* Best before (auto-filled, editable) */}
                            <td style={{ padding:'4px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                ...
                <div style={{ padding:'10px 14px', display:'flex', gap:'10px', alignItems:'center', background:'#F9FAFB', borderTop:'1px solid #F1F5F9', flexWrap:'wrap' }}>
                  ...
                  <span style={{ fontSize:'11px', color:'#94A3B8' }}>Batch number and best before date auto-fill based on brand, pack size, date and shift</span>
                            ...
                            {records.map((r,ri)=>{
                              ...
                              return (
                                <tr key={r.id} style={{ background:wFail?'#FFF8F0':rowBg }}>
                                  <td style={{ padding:'9px 10px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', fontWeight:'600', color:'#475569' }}>{r.line_number||'—'}</td>
                                  ...
                                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', textAlign:'center' }}>
                                    <span style={{ background:'#FFFBEB', color:AM, padding:'2px 7px', borderRadius:'5px', fontSize:'11px', fontWeight:'700', border:'1px solid #FED7AA' }}>{r.pack_size}</span>
                                  </td>
