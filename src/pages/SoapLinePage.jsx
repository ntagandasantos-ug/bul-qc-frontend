...
// Soap Line Inspection — tabular layout, line per brand
// ============================================================
...
function SmartCell({ paramKey, value, onChange, options, onNewOption, placeholder }) {
  ...
  return (
    <div ref={ref} style={{ position:'relative', minWidth:'90px' }}>
      <input
        type="text" value={q}
      ...
      {open && filtered.length > 0 && (
        <div style={{ position:'absolute', top:'100%', left:0, right:0, background:'#fff', border:'1.5px solid #E2E8F0', borderRadius:'8px', boxShadow:'0 4px 16px rgba(0,0,0,0.12)', zIndex:300, maxHeight:'150px', overflowY:'auto', minWidth:'140px' }}>
          {filtered.map(o => (
...
function WeightCell({ packSize, w1, w2, onW1, onW2 }) {
  ...
  const wInp = (val, cb, pass) => (
    <input type="number" value={val ?? ''} onChange={e=>cb(e.target.value)}
      placeholder="g"
      style={{ width:'62px', border:`1px solid ${pass===null?'#E2E8F0':pass?'#86EFAC':'#FECACA'}`, borderRadius:'5px', padding:'5px 6px', fontSize:'12px', fontFamily:'inherit', background:pass===null?'#fff':pass?'#F0FDF4':'#FFF5F5', outline:'none', textAlign:'center' }}
    />
  );
...
export default function SoapLinePage() {
  ...
  const handleSave = async () => {
    ...
    if (form.brands.some(b=>!b.brand_name||!b.pack_size)) { toast.warning('Select brand and pack size for all rows'); return; }
    setSaving(true);
    try {
      ...
      for (const b of form.brands) {
        ...
        const wPass = (!isNaN(w1)&&!isNaN(w2)) ? (w1>=spec.min&&w1<=spec.max&&w2>=spec.min&&w2<=spec.max) : null;
        await supabase.from('soap_inspection_brand_records').insert({
          entry_id:entry.id, brand_name:b.brand_name, pack_size:b.pack_size,
          appearance:b.appearance||null, texture:b.texture||null, specs:b.specs||null,
          color:b.color||null, stamp:b.stamp||null, perfume:b.perfume||null,
          cutting:b.cutting||null, dimensions:b.dimensions||null,
          weight_1:isNaN(w1)?null:w1, weight_2:isNaN(w2)?null:w2,
          weight_pass:wPass, remarks:b.remarks||null, action_taken:b.action_taken||null,
          ...
          <>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'14px' }}>
              ...
              <button onClick={()=>setShowForm(!showForm)}
                style={{ padding:'9px 20px', background:showForm?'#F1F5F9':`linear-gradient(135deg,${P},${PM})`, color:showForm?'#475569':'#fff', border:showForm?'1.5px solid #E2E8F0':'none', borderRadius:'9px', fontSize:'13px', fontWeight:'700', cursor:'pointer', fontFamily:'inherit' }}>
            ...
            {/* ═══ ENTRY FORM ═══ */}
            {showForm && (
              <div style={{ background:'#fff', borderRadius:'14px', border:`2px solid ${PL}`, marginBottom:'18px', overflow:'hidden', boxShadow:'0 4px 16px rgba(107,33,168,0.07)' }}>
                <div style={{ padding:'14px 18px', borderBottom:'1px solid #F1F5F9', background:'#F5F3FF', display:'flex', gap:'16px', alignItems:'center', flexWrap:'wrap' }}>
                  <div>
                  ...
                  <table style={{ borderCollapse:'collapse', width:'100%', fontSize:'12px' }}>
                    <thead>
                      <tr>
                        <TH w="90px">Line</TH>
                        <TH w="150px">Brand</TH>
                        <TH w="75px">Pack Size</TH>
                        {PARAMS.map(p=><TH key={p.key} w="90px" center>{p.label}</TH>)}
                        <TH w="150px" center>Weight (Bar1 / Bar2)</TH>
                        <TH w="110px">Remarks</TH>
                        <TH w="110px">Action Taken</TH>
                        <TH w="40px" center></TH>
                      </tr>
                    </thead>
                    <tbody>
                      {form.brands.map((b, idx) => {
                        ...
                        return (
                          <tr key={idx} style={{ background:evenBg }}>
                            {/* Line */}
                            <td style={{ padding:'6px 6px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                              <select value={b.line_number} onChange={e=>setB(idx,'line_number',e.target.value)}
                                style={{ width:'100%', border:'1px solid #E2E8F0', borderRadius:'5px', padding:'5px 4px', fontSize:'12px', fontFamily:'inherit', background:'#fff', outline:'none', cursor:'pointer' }}>
                            ...
                            <td style={{ padding:'6px 6px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9' }}>
                              <select value={b.pack_size} onChange={e=>setB(idx,'pack_size',e.target.value)} disabled={!b.brand_name}
                                style={{ width:'100%', border:'1px solid #E2E8F0', borderRadius:'5px', padding:'5px 4px', fontSize:'12px', fontFamily:'inherit', background:b.brand_name?'#fff':'#F8FAFC', outline:'none', cursor:b.brand_name?'pointer':'not-allowed' }}>
                                <option value="">—</option>
                            ...
                            <td style={{ padding:'4px 6px', borderBottom:'1px solid #F1F5F9', textAlign:'center' }}>
                              {form.brands.length>1 && (
                                <button onClick={()=>setForm(p=>({...p,brands:p.brands.filter((_,i)=>i!==idx)}))}
                                  style={{ background:'none', border:'none', color:'#CBD5E1', cursor:'pointer', fontSize:'16px', fontFamily:'inherit', lineHeight:1 }}
                ...
                {/* Add row + Save */}
                <div style={{ padding:'10px 14px', display:'flex', gap:'10px', alignItems:'center', background:'#F9FAFB', borderTop:'1px solid #F1F5F9' }}>
                  ...
                  <button onClick={handleSave} disabled={saving}
                    style={{ marginLeft:'auto', padding:'9px 24px', background:saving?'#A78BFA':`linear-gradient(135deg,${P},${PM})`, color:'#fff', border:'none', borderRadius:'9px', fontSize:'13px', fontWeight:'700', cursor:saving?'not-allowed':'pointer', fontFamily:'inherit' }}>
            ...
            {entries.length === 0 && !showForm ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                {entries.map((entry) => {
                  ...
                  return (
                    <div key={entry.id} style={{ background:'#fff', borderRadius:'12px', border:`2px solid ${wOOS?RD:PL}`, overflow:'hidden', boxShadow:wOOS?'0 2px 8px rgba(220,38,38,0.08)':'0 1px 4px rgba(107,33,168,0.05)' }}>
                      {/* Entry header */}
                      <div style={{ background:wOOS?'#FEF2F2':'#F5F3FF', padding:'10px 16px', display:'flex', alignItems:'center', gap:'14px', borderBottom:`1px solid ${wOOS?'#FECACA':PL}`, flexWrap:'wrap' }}>
                            ...
                            {brands.map((b, bi) => {
                              ...
                              return (
                                <tr key={b.id} style={{ background:wFail?'#FFF5F5':rowBg }}>
                                  <td style={{ padding:'9px 10px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', fontWeight:'600', color:'#475569' }}>
                                    {b.line_number||'—'}
                                  </td>
                                  <td style={{ padding:'9px 10px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', fontWeight:'700', color:'#0F172A', whiteSpace:'nowrap' }}>
                                    {b.brand_name}
                                  </td>
                                  ...
                                  <td style={{ padding:'9px 10px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', textAlign:'center' }}>
                                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'2px' }}>
                                      ...
                                      <span style={{ fontSize:'9px', color:wFail?RD:'#94A3B8', fontWeight:wFail?'700':'400' }}>
                                        {wFail ? '⚠️ OOS — Spec: '+spec.label : b.weight_pass===true ? '✓ PASS — '+spec.label : spec.label}
                                      </span>
                                    </div>
                                  </td>
                                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #F1F5F9', borderRight:'1px solid #F1F5F9', color:b.remarks?'#1E293B':'#CBD5E1', fontSize:'12px' }}>
                                    {b.remarks||'—'}
                                  </td>
                                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #F1F5F9', color:b.action_taken?'#1E293B':'#CBD5E1', fontSize:'12px' }}>
                                    {b.action_taken||'—'}
                                  </td>
                                </tr>
                              );
            ...
            {(entries.length>0||inspection) && (
              <div style={{ marginTop:'24px', background:'#fff', borderRadius:'14px', border:`2px solid ${PL}`, padding:'22px', boxShadow:'0 2px 8px rgba(107,33,168,0.05)' }}>
